...
extern bool hashmap_scan(struct hashmap *map, bool (*iter)(const void *item, void *udata), void *udata);
extern bool hashmap_iter(struct hashmap *map, size_t *i, void **item);

extern uint64_t hashmap_sip(const void *data, size_t len, uint64_t seed0, uint64_t seed1);
extern uint64_t hashmap_murmur(const void *data, size_t len, uint64_t seed0, uint64_t seed1);
extern uint64_t hashmap_xxhash3(const void *data, size_t len, uint64_t seed0, uint64_t seed1);

extern const void *hashmap_get_with_hash(struct hashmap *map, const void *key, uint64_t hash);
extern const void *hashmap_delete_with_hash(struct hashmap *map, const void *key, uint64_t hash);
extern const void *hashmap_set_with_hash(struct hashmap *map, const void *item, uint64_t hash);
